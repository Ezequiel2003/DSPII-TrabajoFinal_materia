...
while(cap.isOpened()):
    ...
    if ret==True: 
        ...
        dist_izq = "None" #distancia al borde izquierdo de la línea
        dist_der = "None" #distancia al borde derecho de la línea
        cont_izq = 0
        cont_der = 0
        ...
        if(dist_izq == "None" and dist_der == "None"): #hay un corte de línea
            cv2.putText(img_perspectiva,'Despacio, corte de linea',position,font,fontScale,fontColor,thick)
            letra = "a"
        
#         elif(dist_izq != "None" and dist_izq < ancho_aprox and dist_der != "None" and  dist_der < ancho_aprox): #si está dentro la línea, sigue derecho
#             cv2.putText(img_perspectiva,'Seguir derecho',position,font,fontScale,fontColor,thick)
#             letra = "b"
        elif(dist_izq != "None" and dist_izq < 57 and dist_der != "None"): #dentro de la línea negra casi a la izquierda
            cv2.putText(img_perspectiva,'Doblar a la derecha',position,font,fontScale,fontColor,thick)
            cv2.line(img_perspectiva, (int(ancho/2),int(alto/2)), (int(ancho/2) + dist_der -57,int(alto/2)), (255,0,0),thickness = 5) #linea izquierda
            letra = "c"
        
        elif(dist_der != "None" and dist_der < 57 and dist_izq != "None"): #dentro de la línea negra casi a la izquierda
            cv2.putText(img_perspectiva,'Doblar a la izquierda',position,font,fontScale,fontColor,thick)
            cv2.line(img_perspectiva, (int(ancho/2),int(alto/2)), (int(ancho/2) - dist_izq +57,int(alto/2)), (255,0,0),thickness = 5)
            letra = "e"
        
        elif(dist_izq == "None" and dist_der > 1 and dist_der < 240): #si se pasó a la izquierda, doblar a la derecha
            cv2.putText(img_perspectiva,'Doblar a la derecha - medio',position,font,fontScale,fontColor,thick)
            cv2.line(img_perspectiva, (int(ancho/2),int(alto/2)), (int(ancho/2) + dist_der,int(alto/2)), (255,0,0),thickness = 5) #linea izquierda
            letra = "g"
        
        elif(dist_izq == "None" and dist_der > 240): #si se pasó a la izquierda por mucho, doblar a la derecha más brusco 
            cv2.putText(img_perspectiva,'Doblar mas a la derecha',position,font,fontScale,fontColor,thick)
            cv2.line(img_perspectiva, (int(ancho/2),int(alto/2)), (int(ancho/2) + dist_der ,int(alto/2)), (0,255,255),thickness = 5) #linea izquierda
            letra = "d"
        
        elif(dist_der == "None" and dist_izq > 1 and dist_izq < 240): #si se pasó a la derecha, doblar a la izquierda
            cv2.putText(img_perspectiva,'Doblar a la izquierda - medio',position,font,fontScale,fontColor,thick)
            cv2.line(img_perspectiva, (int(ancho/2),int(alto/2)), (int(ancho/2) - dist_izq ,int(alto/2)), (255,0,0),thickness = 5)
            letra = "h"
        
        elif(dist_der == "None" and dist_izq > 240): #si se pasó a la derecha por mucho, doblar a la izquierda más brusco
            cv2.putText(img_perspectiva,'Doblar mas a la izquierda',position,font,fontScale,fontColor,thick)
            cv2.line(img_perspectiva, (int(ancho/2),int(alto/2)), (int(ancho/2) - dist_izq ,int(alto/2)), (0,255,255),thickness = 5)
            letra = "f" 
